...
"""
class VAE(nn.Module):
    def __init__(self, imgChannels=1, featureDim=32*20*20, hiddenDim=256, zDim=10):
...
for epoch in range(num_epochs):
    ...
    if epoch == 0 or ((epoch + 1) % 25 == 0):
        net.eval()
        net.training = False
        torch.save(net.state_dict(), 'beta/fashion20{}Epoch_{}.pt'.format(beta, epoch))
        with torch.no_grad():
            for data in random.sample(list(test_loader), 1):
                ...
                plt.imshow(np.squeeze(outimg), cmap="gray")
                # plt.imshow(cv2.cvtColor(np.squeeze(outimg), cv2.COLOR_BGR2RGB))
                plt.savefig('beta/addfsadfssf20{}Epoch_{}.png'.format(beta, epoch))
